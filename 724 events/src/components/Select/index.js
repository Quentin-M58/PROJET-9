...
const Select = ({
  ...
  // ajout Plusstyle pour positionner input
  const Plusstyle = {
    opacity: 0,
    position: 'relative',
    left: 50,
  ...
  const changeValue = (newValue) => {
    // ajout newvalue a onchange pour transmettre la valeur
    onChange(newValue);
    setValue(newValue);
      ...
      <div className="Select">
        <ul>
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"} >
            {/* ajout "Veuillez choisir" pour texte en attente de saisie */}
            {value || (!titleEmpty && "Toutes") || "Veuillez choisir"}
          </li>
          {!collapsed && (
            <>
              {!titleEmpty && (
        ...
        {/* modification de valeu par default value pour definir la valeur par default */}
        {/* ajout style pour permettre un positionnement de input */}
        {/* ajout required pour definir sur obligatoire */}
        <input type="text" defaultValue={value || ""} name={name} style={Plusstyle} required />
        <button
          type="button"
