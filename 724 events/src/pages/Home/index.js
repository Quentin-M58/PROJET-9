...
const Page = () => {
  // const {last} = useData()
  // modification de last par data car bonne valeur
  const { data } = useData()
  // ajout const[...]1)[0]; car besion de definir last   
  // sort vient trier les elements en fonctions de la date
  // slice vient recupere le dernier element
  ...
  return <>
    ...
    <footer className="row" data-testid="footer">
      <div className="col presta">
        ...
        {/* ajout {last && ( pour avoir la derniere  */}
        {last && (
          <EventCard
            imageSrc={last?.cover}
            title={last?.title}
            ...
            small
            // label="boom"
            // modification de "boom" par {last?.type} pour avoir le type
            label={last?.type}
            // ajout repere pour test
